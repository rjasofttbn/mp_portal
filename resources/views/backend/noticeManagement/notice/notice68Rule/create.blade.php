    ...
    <div class="row">
        ...
        <div class="col-sm-12 col-md-10 col-lg-10">
            <select id="mp_list" name="mp_list[]" multiple='multiple' class="@error('mp_list') is-invalid @enderror form-control form-control-sm select2">
                ...
                @if (isset($allProfileData) && count($allProfileData) > 0)
                @foreach ($allProfileData as $data)
                <option value="{{ $data->user_id }}" {{ in_array($data->user_id, old('mp_list') ?? old('mp_list') ?? [] ) ? 'selected' : ''}}>{{ $data->name_bn }} ({{ $data->voter_area }})</option>
                @endforeach
                @endif
            </select>
            @error('mp_list')
...
<script>
    $(function() {
        ...
        $('#mp_list').select2({
            placeholder: '@lang("Select Maximum Five(5) MP Name")',
            allowClear: true,
            multiple: true,
            maximumSelectionLength: 5,
        });

        $('#noticeCreateForm').on('submit', function() {
            var minimum = 5;

            if ($("#mp_list").select2('data').length < minimum) {
