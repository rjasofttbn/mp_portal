...
<div class="content-header">
    <div class="container-fluid">
        <div class="row mb-2">
            <div class="col-sm-6">
                <h4 class="m-0 text-dark">@lang('Notice Stages')</h4>
            </div><!-- /.col -->
            <div class="col-sm-6">
                <ol class="breadcrumb float-sm-right">
                    <li class="breadcrumb-item"><a href="#">@lang('Home')</a></li>
                    <li class="breadcrumb-item active">@lang('Notice Stage')</li>
                </ol>
            </div><!-- /.col -->
        </div><!-- /.row -->
    </div><!-- /.container-fluid -->
    ...
    <div class="col-md-12">
        <div class="card">
            ...
            <form id="stageForm" name="stageForm" method="POST" @if($existingData->id)
                action="{{ route('admin.notice_management.noticestage.update', $existingData->id) }}">
                <input name="_method" type="hidden" value="PUT">
                @else
                action="{{ route('admin.notice_management.noticestage.store') }}">
                @endif
                @csrf
                    ...
                    <div class="row">
                        ...
                        <div class="col-12">
                            <div class="form-group">
                                <label class="control-label" for="rule_number">@lang('Rule')<span style="color: red;"> *</span></label>
                                <select id="rule_number" name="rule_number" class="form-control @error('rule_number') is-invalid @enderror select2">
                                    <option value="">@lang('Select Rule')</option>
                                    @foreach ($parliament_rule_list as $list)
                                    @if($list->rule_number==$existingData->rule_number or $list->rule_number==old('rule_number'))
                                    <option selected value="{{$list->rule_number}}">{{$list->name}}</option>
                                    @else
                                    <option value="{{$list->rule_number}}">{{$list->name}}</option>
                                    @endif
                                    @endforeach
                                </select>
                        ...
                        <div class="col-sm-12">
                            <div class="form-group text-right">
                                ...
                                <button type="button" class="btn btn-default btn-sm ion-android-arrow-back">
                                    <a href="{{route('admin.notice_management.noticestage.index') }}">@lang('Back')</a>
                                </button>
                            </div>
                        </div>
                    </div>
