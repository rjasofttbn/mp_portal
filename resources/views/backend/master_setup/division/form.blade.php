        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    @if ($data->id)
                    <h4 class="card-title">@lang('Update Division')</h4>
                    @else
                    <h4 class="card-title">@lang('Create Division')</h4>
                    @endif
                </div>
                <!-- Form Start-->
                <form id="divisionForm" name="divisionForm" method="POST"
                      @if($data->id)
                      action="{{ route('admin.master_setup.divisions.update', $data->id) }}">
                    <input name="_method" type="hidden" value="PUT">
                    @else
                        action="{{ route('admin.master_setup.divisions.store') }}">
                    @endif
                    @csrf
                            ...
                            <div class="col-sm-12">
                                <div class="form-group text-right">
                                    <button type="button" class="btn btn-default btn-sm ion-android-arrow-back">
                                        <a href="{{route('admin.master_setup.divisions.index') }}">@lang('Back')</a>
                                    </button>
                                    @if($data->id)
