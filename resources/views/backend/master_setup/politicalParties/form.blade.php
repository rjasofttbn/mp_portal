...
@section('content')
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h4 class="m-0 text-dark">@lang('Political Party Management')</h4>
                </div><!-- /.col -->
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="#">@lang('Home')</a></li>
                        <li class="breadcrumb-item active">@lang('Political Party Management')</li>
                    </ol>
                </div><!-- /.col -->
            </div><!-- /.row -->
        </div><!-- /.container-fluid -->
        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    @if ($data->id)
                        <h4 class='card-title'>@lang('Update Political Party')</h4>
                    @else
                        <h4 class='card-title'>@lang('Create Political Party')</h4>
                    @endif
                </div>
                <!-- Form Start-->
                <form id="partiesForm" name="partiesForm" method="POST"
                      @if($data->id)
                      action="{{ route('admin.master_setup.political_parties.update', $data->id) }}">
                    <input name="_method" type="hidden" value="PUT">
                    @else
                        action="{{ route('admin.master_setup.political_parties.store') }}">
                    @endif
                    @csrf
                            ...
                            <div class="col-sm-12">
                                <div class="form-group text-right">
                                    ...
                                    <button type="button" class="btn btn-default btn-sm ion-android-arrow-back">
                                        <a href="{{route('admin.master_setup.political_parties.index') }}">@lang('Back')</a>
                                    </button>
                                </div>
                            </div>
                        </div>
