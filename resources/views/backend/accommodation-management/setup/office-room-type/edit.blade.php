        ...
        <div class="card">
            ...
            <div class="card-body">
                <form id="submitForm">
                    ...
                    @endif
                    @csrf 
                {{-- <form method="POST" action="{{ route('admin.accommodation-management.setup.office_room_types.update', $data->id) }}" enctype="multipart/form-data">
                    @csrf
                    @if (isset($data))
                    @method('PUT')
                    @endif --}}
                            ...
                            <div class="col-sm-4">
                                <div class="form-group">
                                    ...
                                    <input type="text" id="service_charge" name="service_charge" value="{{old('service_charge', $data->service_charge)}}"
                                           class="form-control @error('service_charge') is-invalid @enderror service_charge"
    ...
    $(document).ready(function(){
        $('#submitForm').validate({
            ...
            submitHandler: function (form) {
                ...
                $.ajax({
                    url : "{{(@$data)?route('admin.accommodation-management.setup.office_room_types.update',@$data['id']):route('admin.accommodation-management.setup.office_room_types.store') }}",
                    data : formInfo,
                    type : "POST",
                    processData: false,
                    contentType: false,
