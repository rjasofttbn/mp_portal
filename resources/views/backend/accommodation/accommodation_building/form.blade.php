...
@section('content')
    ...
    <div class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h4 class="m-0 text-dark">@lang('Accommodation Building Management')</h4>
                </div><!-- /.col -->
                <div class="col-sm-6">
                    <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href="#">@lang('Home')</a></li>
                        <li class="breadcrumb-item active">@lang('Accommodation Building Management')</li>
                    </ol>
                </div><!-- /.col -->
            </div><!-- /.row -->
        </div><!-- /.container-fluid -->
        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    ...
                    <h5>@lang('Update Accommodation Building')</h5>
                    @else
                    <h5>@lang('Create Accommodation Building')</h5>
                    @endif
                </div>
                <!-- Form Start-->
                <form  id="submitForm">
                            ...
                            <div class="col-sm-4">
                                <div class="form-group">
                                    <label class="control-label" for="total_floor">@lang('Total Floor')<span
                                            style="color: red;"> *</span></label>
                                    <input type="number" id="total_floor" name="total_floor" value="{{ old('total_floor') ?? $editData->total_floor ?? '' }}"
                                           class="form-control total_floor @error('total_floor') is-invalid @enderror"
                                           placeholder="@lang('Enter Total Floor')" autocomplete="off" >
                                    @error('total_floor')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
                        ...
                        <div class="row">
                            <div class="col-sm-12">
                                <div class="form-group text-right">
                                    ...
                                    @endif
                                    <a href="{{route('admin.accommodation-management.setup.accommodationbuildings.index') }}" class="btn btn-default btn-sm ion-android-arrow-back text-blue"> @lang('Back')</a>
        ...
        $(document).ready(function(){
            $('#submitForm').validate({
                ...
                submitHandler: function (form) {
                    ...
                    $.ajax({
                        url : "{{(@$editData)?route('admin.accommodation-management.setup.accommodationbuildings.update',@$editData->id):route('admin.accommodation-management.setup.accommodationbuildings.store') }}",
                        data : formInfo,
                        type : "POST",
                        processData: false,
                        contentType: false,
                        ...
                        success:function(data){
                            if(data.status == 'success'){
                                ...
                                $('.preload').hide();
                                setTimeout(function(){
                                   location.replace("{{route('admin.accommodation-management.setup.accommodationbuildings.index')}}");
                               }, 1450);
            ...
            jQuery.validator.addClassRules({
                'name' : {
                    ...
                    remote: {
                        ...
                        url: "{{route('admin.accommodation-management.setup.accommodationbuildings.duplicate-check')}}",
                        type: "GET",
                        data: {
